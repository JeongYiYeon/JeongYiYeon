...
public class HeroCharacter : BaseCharacter
{
    ...
    {
        InitStat(UserData.Instance.user.Character.GetHeroCharacter());

        CharacterAnimation.SetAtlas(CharacterAtlasName);
    ...
    private IEnumerator IECheckEnemy()
    {
        while (true)
        {
            ...
            if (IsCollision == true)
            {
                if (isAttack == false)
                {
                    if (GameManager.Instance.GameSpeed > 0 && GameManager.Instance.IsWaveWait == false)
                    {
                        ...
                        Messenger<Vector3, GameObject>.Broadcast(ConstHelper.MessengerString.MSG_ACTIVESKILL,
                            colTargetPos, colGo, MessengerMode.DONT_REQUIRE_LISTENER);
            ...
            {
                Messenger<Vector3, GameObject>.Broadcast(ConstHelper.MessengerString.MSG_ACTIVESKILL,
                    Vector3.zero, null, MessengerMode.DONT_REQUIRE_LISTENER);
            }
        }
    }    
}
